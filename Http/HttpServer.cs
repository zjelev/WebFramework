...
#pragma warning restore CS4014
    ...
    private async Task ProcessClientAsync(TcpClient tcpClient)
    {
        ...
        try
        {
            byte[] requestBytes = new byte[500000]; // TODO: Use buffer
            int bytesRead = await networkStream.ReadAsync(requestBytes, 0, requestBytes.Length);
            string requestAsString = Encoding.UTF8.GetString(requestBytes, 0, bytesRead);
            var request = new HttpRequest(requestAsString);
            //var clientIpLAN = tcpClient.Client.LocalEndPoint; // locally returns "127.0.0.1:[YourServerPort]"
            var clientIpWAN = tcpClient.Client.RemoteEndPoint; // locally returns "127.0.0.1:[totally random port]"
