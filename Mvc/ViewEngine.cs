...
{methodCode}
        ...
        string html = view.GetHtml(model, user, "bg-BG");
        return html;
    }
    ...
    {
        var iviewReference = MetadataReference.CreateFromFile(typeof(IView).Assembly.Location);
        ...
        var compilation = CSharpCompilation.Create("AppViewAssembly")
            ...
            .AddReferences(iviewReference)
            .AddReferences(objectReference);
            
        AssemblyName[] modelReferences = null;
        if (model != null)
        {
            PortableExecutableReference modelReference = MetadataReference.CreateFromFile(model.GetType().Assembly.Location);
            compilation = compilation.AddReferences(modelReference);
            modelReferences = model.GetType().Assembly.GetReferencedAssemblies();
        ...
        while ((line = reader.ReadLine()) != null)
        {
            ...
            else
            {
                ...
                while (line.Contains("@@"))
                {
                    ...
                    var cSharpAndEndOfLine = line.Substring(atSignLocation + 2);
